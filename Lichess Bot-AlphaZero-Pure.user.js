...
function setupWebSocketHandlers(wrappedWebSocket) {
    ...
    wrappedWebSocket.addEventListener("close", function (event) {
        ...
        // CRITICAL: If connection closed due to error (like invalid move), clear stale data
        if (event.code === 1011 || event.reason === "unexpected message") {
            console.log("⚠️ Connection closed due to error - clearing stale FEN data");
            currentFen = ""; // Clear stale FEN - will get fresh data on reconnect
    ...
    wrappedWebSocket.addEventListener("message", function (event) {
        ...
        if (message.d && typeof message.d.fen === "string" && typeof message.v === "number") {
            ...
            // CRITICAL: Determine color and whether to move
            if (myColor === null) {
                // First time seeing a position - try to determine our color
                // In standard games, bot should move automatically
                // Only calculate if we haven't made a move yet
                
                if (message.v === 0) {
                    // Starting position - we play White and should move first
                    myColor = 'w';
                    console.log(`🎮 Starting position - we are WHITE`);
                    calculateMove();
                } else if (message.v === 1) {
                    // One move made - either user played White (we're Black) or we need to detect
                    // If White just moved, we're Black (and should move now)
                    // Check if it's Black's turn
                    if (currentTurnColor === 'b') {
                        myColor = 'b';
                        console.log(`🎮 Detected we are BLACK (opponent played first)`);
                        calculateMove();
                    } else {
                        // This shouldn't happen at v=1, but handle it
                        console.log(`⚠️ Unexpected: v=1 but White to move?`);
                        myColor = 'w';
                        calculateMove();
                    }
                } else {
                    // Multiple moves made - we don't know our color yet
                    // Try to calculate and we'll determine color when we send a move
                    console.log(`🎯 Position received (v=${message.v}) - attempting to calculate...`);
                    calculateMove();
                }
            } else {
                ...
                if (isOurTurn) {
                    console.log(`✅ Our turn (${myColor === 'w' ? 'White' : 'Black'}) - calculating...`);
                    calculateMove();
                } else {
...
function calculateMove() {
    ...
    // HYBRID APPROACH: Use BOTH depth and movetime for reliability
    // Engine will search to depth OR until time runs out, whichever comes first
    // This preserves AlphaZero's deep search while preventing hangs
    let maxMoveTime = thinkTime;
    if (depth >= 22) maxMoveTime = Math.min(thinkTime * 2.5, 15000); // Up to 15s for ultra-deep
    else if (depth >= 20) maxMoveTime = Math.min(thinkTime * 2.0, 12000); // Up to 12s for deep
    else if (depth >= 18) maxMoveTime = Math.min(thinkTime * 1.8, 10000); // Up to 10s for standard
    
    chessEngine.postMessage(`go depth ${depth} movetime ${maxMoveTime}`);
    console.log(`⏱️ Max time: ${(maxMoveTime/1000).toFixed(1)}s for depth ${depth}`);
    
    // Set a generous safety timeout as final fallback (only for true hangs)
    const safetyTimeout = setTimeout(() => {
        if (isCalculating) {
            console.warn("⚠️ Engine hung - forcing emergency stop!");
            chessEngine.postMessage("stop");
            isCalculating = false;
        }
    }, maxMoveTime + 5000); // 5s grace period after engine's own timeout
    
    // Store timeout ID to clear it if move completes normally
    window.lastSafetyTimeout = safetyTimeout;
}
...
function setupChessEngineOnMessage() {
    ...
    chessEngine.onmessage = function (event) {
        ...
        if (event && event.includes("bestmove")) {
            ...
            if (window.lastSafetyTimeout) {
                clearTimeout(window.lastSafetyTimeout);
                window.lastSafetyTimeout = null;
            }
