...
function setupWebSocketHandlers(wrappedWebSocket) {
    ...
    wrappedWebSocket.addEventListener("message", function (event) {
        ...
        if (message.d && typeof message.d.fen === "string" && typeof message.v === "number") {
            ...
            if (myColor === null) {
                // CRITICAL FIX: At starting position (v=0) or when White to move (v even), assume we're WHITE
                // This ensures the bot plays as WHITE from the start
                if (message.v === 0) {
                    myColor = 'w';
                    console.log(`🎮 Starting position detected - playing as WHITE`);
                } else if (currentTurnColor === 'w') {
                    // If it's White's turn and we haven't determined color, we're probably WHITE
                    myColor = 'w';
                    console.log(`🎮 White to move detected - assuming we are WHITE`);
                } else {
                    // If it's Black's turn and we haven't determined color, we're probably BLACK
                    myColor = 'b';
                    console.log(`🎮 Black to move detected - assuming we are BLACK`);
                }
                
                // Now calculate if it's our turn
                if (currentTurnColor === myColor) {
                    console.log(`✅ It's OUR turn (${myColor === 'w' ? 'White' : 'Black'}) - calculating first move...`);
                    calculateMove();
                } else {
                    console.log(`⏳ Opponent's turn - waiting for their move...`);
                }
            } else {
