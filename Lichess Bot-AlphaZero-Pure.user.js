...
let lastCalculatedFen = ""; // Track which position we last calculated
let lastSentMove = null; // Track the last move we sent
let moveSentTimestamp = 0; // When we sent the last move
let calculationDebounceTimer = null; // Prevent rapid recalculations
...
function setupWebSocketHandlers(wrappedWebSocket) {
    ...
    wrappedWebSocket.addEventListener("open", function () {
        ...
        reconnectionRecoveryTimer = setTimeout(() => {
            // CRITICAL: Don't recalculate if we just sent a move recently
            const timeSinceLastMove = Date.now() - moveSentTimestamp;
            if (moveSentTimestamp > 0 && timeSinceLastMove < 2000) {
                console.log(`⏳ Just sent move ${timeSinceLastMove}ms ago, waiting for fresh position...`);
                return;
            }
            ...
            if (currentFen && !isCalculating) {
                ...
                // CRITICAL: Check if this is the same position we already calculated
                if (currentFen === lastCalculatedFen) {
                    console.log("⏭️ Already calculated this position before reconnection, skipping");
                    return;
                }
                ...
                // Only recalculate if it's OUR turn AND it's a new position
        ...
        }, 2000); // Wait 2s for things to stabilize (increased from 1.5s)
    });
    ...
    wrappedWebSocket.addEventListener("message", function (event) {
        ...
        if (message.d && typeof message.d.fen === "string" && typeof message.v === "number") {
            ...
            // CRITICAL: Check if this is a duplicate position
            if (currentFen === lastCalculatedFen) {
                console.log(`⏭️ Already calculated this position (v=${message.v}), skipping duplicate`);
                return;
            }
...
function startWebSocketMonitor() {
    ...
    wsStateCheckInterval = setInterval(() => {
        ...
        if (webSocketWrapper && (webSocketWrapper.readyState === 2 || webSocketWrapper.readyState === 3)) {
            // Don't log repeatedly - only log once per stale detection
            // The proxy will catch new connections automatically
        }
...
function calculateMove() {
    ...
    // CRITICAL: Check if we already calculated this exact position
    if (currentFen === lastCalculatedFen) {
        console.log("⏭️ Already calculated this position, skipping duplicate calculation");
        return;
    }
    
    // CRITICAL: Don't calculate too soon after sending a move (debounce)
    const timeSinceLastMove = Date.now() - moveSentTimestamp;
    if (moveSentTimestamp > 0 && timeSinceLastMove < 1000) {
        console.log(`⏳ Too soon after last move (${timeSinceLastMove}ms), waiting for opponent...`);
        return;
    }
    ...
    // Debounce rapid calculation requests (wait 300ms)
...
function performCalculation() {
    // Mark this position as calculated
...
function sendMove(move, retryCount = 0) {
    ...
    setTimeout(() => {
        ...
        try {
            ...
            // CRITICAL: Track that we sent this move
            lastSentMove = move;
            moveSentTimestamp = Date.now();
            
            // CRITICAL: Clear the calculated position to allow new calculations
            // We expect a new position after opponent moves
            lastCalculatedFen = "";
